...
class ModelTest(TestCase):
    def setUp(self):
        Event.objects.create(id=1, name='oneplue3 event',
                 status=True, limit=2000,
                 address='shenzhen', start_time='2019-08-23 03:18:22')
    ...
    def test_event_models(self):
        result = Event.objects.get(name='oneplue3 event')
        self.assertEqual(result.address, 'shenzhen')
        self.assertTrue(result.status)

    def test_guest_models(self):
        result = Guest.objects.get(phone='123456')
        self.assertEqual(result.realname, 'xiaoming')
...
class IndexPageTest(TestCase):
    ...
    def test_index_page_renders_index_template(self):
        ...
        # 上面虽然没有导入Client方法， 但是self.client默认调用django.tets.Client类

        self.assertEqual(response.status_code, 200)

        # 断言服务器是否用给定的是index.html模板响应
...
class LoginActionTest(TestCase):
    ...
    def test_login_action_success(self):
        ''' 登录成功 '''
        response = self.client.post('/login_action/', data={'username': 'admin', 'password': '123456'})
